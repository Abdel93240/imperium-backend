...
| events | 0011,0029,0030,0031 | (c) divergent — Append-only DB SOLIDE (`events`, `auth_events`, `imperium_events` bloquent UPDATE/DELETE/TRUNCATE) et idempotence OK. MAIS deux journaux actifs coexistent : `events` = enveloppe EVENT-005 quasi complète + vrais events métier dotted ; `imperium_events` = fondation `/api/imperium/events`, enveloppe réduite, snake_case, pas utilisée par les services métier. Contradiction docs : `08/06` exigent dotted, `04/05` verrouillent snake_case. `ai.result.stored` documenté mais non émis. Vault récent canonique n'émet pas d'event, seul le vieux `/api/vault` émet `vault.transaction.created`. | trancher le journal canonique unique; garder probablement `events` ou migrer `imperium_events` vers enveloppe EVENT-005+dotted; rebrancher services métier; ajouter `ai.result.stored`; aligner docs/tests | audité — CAMPAGNE COMPLÈTE 11/11 |
