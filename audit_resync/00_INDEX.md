...
| weekly_review | 0010,0013,0014,0015,0016 | WR-a (SCHÉMA) : (b) léger décalage — schéma WR SAIN et fidèle au doc 32. Session backend-owned, messages ordonnés, rapports historisés (`draft`/`approved`/`stored`/`superseded`), décisions mémoire SÉPARÉES des mémoires (principe "décision ≠ mémoire" respecté, AUCUN write auto en mémoire). Pas de `pgvector_memory`. Petits écarts : rôles messages contiennent `qwen`/`opus` (nomenclature à généraliser), `analysis_status` vestige, vocabulaire "active" flou. ⚠️ DÉPENDANCE CLÉ : le lien WR→mémoire pointe vers `ai_memories` en mode WR-spécifique (`source_decision_id`/`report_id`/`session_id`), PAS le générique `source_table`/`source_id` du doc 75. Donc le WR est prisonnier d'`ai_memories`. WR-b (ÉTATS) : (c) divergent DE PÉRIMÈTRE — `weekly_review_state.py` n'est PAS la machine à états du WR (malentendu de nom). C'est une couche BANNER/READINESS séparée (table `imperium_weekly_review_states` : `ready`/`launched`/`analysis_status`). La VRAIE machine à états (`preparing`→`conversation`→`draft`→`approved`→`stored`) vit dans `weekly_review_conversation.py` (à auditer en WR-c). Constats : (a) fichier mal nommé (laisse croire qu'il porte la machine à états) → renommer en readiness/banner ; (b) `analysis_status` = vestige ancien modèle "analysis" → déprécier ; (c) le timing mardi 20h n'est PAS imposé par le backend, il dépend d'un scheduler externe → à documenter ; (d) unicité (user, semaine) OK, pas de noms de modèles concrets ici. La machine à états réelle reste à auditer (WR-c). Statut : WR-a + WR-b audités. Reste : WR-c (logique conversation, machine à états réelle). | ne pas toucher au schéma WR avant WR-c; corriger `ai_memories` avant le commit mémoire WR; décider si `imperium_weekly_review_states` reste une couche readiness ou doit être supprimée/rebranchée sur sessions; documenter le schéma WR dans 05 ou doc 32 | schéma audité (WR-a), WR-b états audité. Reste : WR-c (logique conversation, 3728 l., découpée en blocs) |
