...
| ai_tasks_results | 0012 | (c) divergent de périmètre — fondation storage/callback SAINE mais pas encore le cerveau IA cible. Code conforme à l'esprit Patch 2A/2E : `ai_tasks` queued, `ai_results` pending_validation, validations explicites, HMAC callback, idempotency, aucun write canonique automatique. Écarts majeurs vs doc 31 §7 + doc 16 §4/4A : schéma V1 complet non codé (`difficulty_score`, `score_breakdown`, `routing_model`, `selected_model`, `fallback_model`, `requires_user_validation`, hashes/cost/tokens/latency...), statuts divergents et `postponed` absent, pas de vrai claim n8n atomique, pas de priorité `INTERACTIVE/BACKGROUND`, pas de file à priorités, pas de verrou de session, pas d'event `ai.result.stored`. Provider Qwen = adaptateur fonctionnel/dry-run + endpoint Ollama possible, mais defaults/prompts/workflows/tests gardent `qwen2.5:7b-instruct` alors que doc 30/31/16 canonisent Qwen 32B / `qwen3:32b`. Pas de `pgvector_memory` actif ici. | coder la queue IA V1 : ajouter priorité + `postponed` + claim interne atomique + session lock; brancher le scoring `/200` Qwen dans le lifecycle; aligner le provider/config/workflows/tests sur Qwen 32B ou alias `local_router`; décider si doc 31 §7 est obligatoire ou backlog futur | audité |
...
Le routage intelligent doc 30 (scoring `/200`, audit entrée/sortie, `router_decision`, choix dynamique de modèle) n'est codé NULLE PART encore dans les modules audités (`daily_plan`, WR...). C'est LA grande couche "cerveau" qui attend le modèle local/GPU.
