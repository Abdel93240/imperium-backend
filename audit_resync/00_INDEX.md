...
| pulse | 0028 | (c) divergent de périmètre — la fondation codée est saine et alignée colonne/ORM/schema (`imperium_pulse_entries`, today read-only, stats, idempotence POST, aucun AI/n8n/cloud/pgvector). MAIS docs 40/34 décrivent un Pulse santé/médical V1 beaucoup plus large (repas, hydratation, stock, workouts, pain logs, documents médicaux, consentement RGPD, règles médicales) non codé. Privacy médicale OK par absence d'upload/appel cloud, pas par garde-fous implémentés. | trancher si V1 Pulse reste `imperium_pulse_entries` minimal ou si doc 34 médical devient obligatoire; si obligatoire, coder consentement/documents/règles/tests avant toute analyse cloud | audité |
| events | 0011,0029,0030,0031 | — | — | à auditer |
| calendar | 0022 | (a) conforme à la fondation Patch 7H — doc 51 est clairement FUTURE/V3, mais autorise une fondation backend minimale. Code aligné : `imperium_calendar_events`, types `event/deadline/vacation`, create/list/delete manuel, scope user, idempotence create, validation dates, aucun AI/n8n/pgvector/sync externe. Le Calendar complet V3 (recurrence, overrides, hooks 7 jours, deadline planner, IA) n'est pas attendu maintenant. Dette doc : noms modèles futurs `Sonnet/Opus` périmés. | garder la fondation telle quelle; ne pas lancer le Calendar V3 avant stabilisation V1/V2; plus tard ajouter audit/hook delete si nécessaire | audité |
| fondation (skeleton/security/guards) | 0001,0002,0003 | (b) base solide, couverture DB partielle — extensions `pgcrypto`/`vector`, user canonique, devices, refresh tokens durcis, idempotency keys, event envelope, auth/events append-only UPDATE/DELETE/TRUNCATE sont en place. Limites : singleton user seulement sur `single_user_mode=true`, pas de check DB dotted `event_type`/payload object dans `events`, privacy gate AI/cloud pas DB-only, règles métier IMP/Vault/Path/Pulse hors de ces 3 migrations. | conserver la base; ajouter des contraintes DB ciblées sur `events` si souhaité; ne pas attendre des migrations 0001-0003 qu'elles portent toutes les règles métier ultérieures | audité |
