...
| weekly_review | 0010,0013,0014,0015,0016 | WR-a (SCHÉMA) : (b) léger décalage — schéma WR SAIN et fidèle au doc 32. Session backend-owned, messages ordonnés, rapports historisés (`draft`/`approved`/`stored`/`superseded`), décisions mémoire SÉPARÉES des mémoires. WR-b (ÉTATS) : (c) divergent DE PÉRIMÈTRE — `weekly_review_state.py` est une couche BANNER/READINESS, pas la machine WR. WR-c (CONVERSATION) : verdict global **(c) sur la mémoire, (b) sur la conversation**. `weekly_review_conversation.py` est bien la vraie machine conversationnelle : guards terminaux, idempotence, n8n signé/dry-run, drafts non canoniques, approve puis store séparés. Mais le bloc mémoire committe déjà réellement vers `ai_memories`, avec dépendance WR-spécifique (`source_decision_id`/`source_report_id`/`source_session_id`, `kind/scope/status/visibility`), sans embedding ni `privacy_level`, donc divergent face aux docs 75/09. Routage doc 30 non visible : pas de score `/200`, pas d'audit entrée/sortie high reasoning model, restes `qwen2.5:7b-instruct`/`qwen`/`opus`. WR complet audité. | corriger `ai_memories` AVANT le commit mémoire WR; rebrancher le commit sur `source_table/source_id`, `privacy_level`, embedding, `memory_type/learning_element_type`; centraliser les transitions WR; remplacer les noms de modèles concrets par des rôles; aligner doc 32 sur l'existence du commit mémoire réel | WR complet audité (WR-a schéma, WR-b readiness/états, WR-c conversation/mémoire) |
