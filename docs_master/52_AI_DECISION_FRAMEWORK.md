...
DECISION TREE:
  ...
  ELSE (conflict or infeasibility):
    ...
    → `local_executor`, via scoring, arbitrates locally (mapping: doc 30 §3).

  IF exceptional full regeneration is required:
    → `high_reasoning` may be used only after user validation (mapping: doc 30 §3).
```
...
conflict or infeasibility, `local_executor` arbitrates. `high_reasoning` is not a
routine quality fallback: it is reserved for an exceptional full regeneration
explicitly validated by the user. Role assignments are defined by doc 30 §3.
