...
- `app/api/v1/routes/imperium_path.py` is canonical for Path V1, including `GET /api/imperium/path/today`.
...
- Legacy `ImperiumPathItem` code must never own `/path/today`; Path V1 today returns `PathTodayResponse`.
