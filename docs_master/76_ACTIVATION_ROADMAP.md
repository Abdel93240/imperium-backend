...
| ACT-SYS-06 | Runner de jobs + heartbeat : premier job actif | dE | 2 | NOT_CODED | V1 | — | socle 0a (T2) |
| ACT-VLT-05 | Pression financière 0-100 + explication | dL | 1 | NOT_CODED | V1 | — | RÉSOLUES Q4/Q17 : échelle 0-100 ; V1 corrigée avec upcoming CRUD |
| ACT-SYS-07 | Canal de notifications produit | No | 3 | NOT_CODED | V1 | — | RÉSOLUES Q3+Q18 : Telegram produit d'abord |
| ACT-VLT-04 | Dépenses récurrentes + upcoming (CRUD) | dE | 2 | NOT_CODED | V1 | — | remonte de V3 pour pression V1 |
| ACT-PTH-02 | Fenêtres de prière (MAWAQIT + fallback) | dL | 1 | NOT_CODED | V1 | — | RÉSOLUES Q6/Q19 : socle §8 |
| ACT-SYS-19 | Backup nocturne restic (job system.backup_nightly) | dE | 2 | NOT_CODED | V1 | — | socle §12 |
...
| ACT-VLT-09 | Profit hebdo business (job lundi 00:30) | dE | 2 | NOT_CODED | V4 | — | RÉSOLUE Q9 : mini-passe Vault |
...
| ACT-PLS-16 | Interprète réel local (shadow de fait) | iaS | 3 | NOT_CODED | V13 | — | RÉSOLUE Q21 : 14 j ET ≥20 sorties auditées |
...
| ACT-WR-21 | Régénération mensuelle | Pr | 4 | NOT_CODED | V24 | — | RÉSOLUE Q8 : plan_versions |
...
| ACT-WR-17 | Moteur d'exposition 6.3 (confiance) | Ap | 5 | NOT_CODED | V25 | — | RÉSOLUE Q5 : spec WR normative, doc 75 amendé au socle |
...
| ACT-SYS-08 | toolbox.travel v0 (Google, plancher 1,3) | dL | 1 | NOT_CODED | V28 | — | RÉSOLUE Q2 : privacy_tier travel |
...
| ACT-DLY-10 | Prières en engagements fixes G4 | dL | 1 | NOT_CODED | V29 | — | RÉSOLUE Q6 : prières G4 via socle §8 |
...
| ACT-VEC-01 | Matrice H3 + caches statiques | dE | 2 | NOT_CODED | V31 | — | RÉSOLUE Q1 : GitHub vérité / clone Tower ; V-2 à vérifier à la passe Vector |
...
| ACT-EVT-04 | Débranchement legacy (4 objets) | dE | 2 | NOT_CODED | V36 | — | RÉSOLUE Q15 : drop à la passe du domaine + dump |
...
ne sont pas confondables (RÉSOLUE Q20 : max 2 vagues, jamais 2× échelon ≥3, chrono de
validation comme juge) ; chaque vague exige le merge (éteint) de la passe qui la code —
indiqué entre crochets.

| vague | contenu | prérequis | durée obs. |
|---|---|---|---|
| V0 | état initial : 8 features déjà ON | — (constat, HYPOTHÈSE prod) | — |
| V1 | boucle vitale : runner+heartbeat, canal notifications, pression, upcoming CRUD ; V1-bis à J+3 : fenêtres de prière + backup | [socle 0a/0b + mini-Vault pression + socle §8/§12] Q17 tranchée | 7 j |
| V2 | hygiène du journal : renommage+E1, chaînage V1, types manquants | [socle §6] AVANT tout consumer pérenne (AD-6) | 3-7 j |
...
| V13 | interprète réel local (shadow de fait : nomme, rien ne s'exécute) | V6, V11, V12 (audit cloud) ; RÉSOLUE Q21 : 14 j ET ≥20 sorties auditées | 14 j + ≥20 sorties |
...
| V25 | mémoire vivante : commit D5, W3 réel, exposition 6.3 | V6 (embeddings), RÉSOLUE Q5 : spec WR normative | 14 j |
...
| V28 | sélection du jour : travel v0, sélection N1, pull-forward | V27 ; RÉSOLUE Q2 : `privacy_tier` travel | 2-3 j |
| V29 | Daily branchements : G5, overrides, agrégation, prières G4 | V28 ; V9 (G5) ; V18 (docket) ; RÉSOLUE Q6 : socle §8 | 3-7 j |
| V30 | Daily N2 : dry-run, disruption, arbitrate, préemption, décroissance | V29, V13 | 7-14 j |
| V31 | Vector Tower : matrice, builders, sync, test notification | [passe Vector] RÉSOLUE Q1 : GitHub vérité / clone Tower ; V-2 résolu à l'étape 0 Vector | 3-7 j |
...
| V36 | nettoyages : drops legacy (Q15), routeur /200 (différable) | merge du socle ; ponts portés §2 | 3-7 j |
| V37+ | Feed AI / Knowledge Inbox | passe dédiée post-Vector (doc 70) | à cadrer |

Horizon cumulé indicatif (chevauchements inter-domaines autorisés, RÉSOLUE Q20 : max 2 vagues,
jamais 2× échelon ≥3, chrono de validation comme juge) : V1-V5 ≈ 4-5
...
| CF-4 | ~25 jobs/crons des 4 specs sans flag d'extinction (features 06:45, factory session_end, scores 06:30, builders 5 min…) → tournent dès que le runner existe | Pulse §14, WR §12, Daily §12, Vector §4.1 | déjà conforme dans les specs Socle §2 / Vault §4 : `job_definitions.enabled=false` par défaut au seed du runner ; activation par job |
...
| CF-6 | profit hebdo « lundi 00:30 » codé comme cron | doc 42 §11 / GAP_vault | déjà conforme dans les specs Socle §2 / Vault §4 : job runner `enabled=false` par défaut (couvert par CF-4, rappelé pour la mini-passe Vault) |
...
## Décisions utilisateur résolues

| question | réponse |
|---|---|
| Q1 | RÉSOLUE : GitHub est la vérité, clone Tower. |
| Q2 | RÉSOLUE : `privacy_tier` travel. |
| Q3+Q18 | RÉSOLUES : Telegram produit d'abord. |
| Q4 | RÉSOLUE : pression 0-100. |
| Q5 | RÉSOLUE : spec WR normative, doc 75 amendé au socle. |
| Q6 | RÉSOLUE : prières G4 via socle §8. |
| Q8 | RÉSOLUE : `plan_versions`. |
| Q9 | RÉSOLUE : mini-passe Vault. |
| Q15 | RÉSOLUE : drop à la passe du domaine + dump. |
| Q16 | RÉSOLUE : les passes avancent ; seule la chaîne mémoire attend le smoke P40. |
| Q17 | RÉSOLUE : V1 contient runner+heartbeat, canal notifications, pression, upcoming CRUD ; V1-bis à J+3 contient fenêtres de prière + backup. |
| Q19 | SANS OBJET : c'est le socle §8. |
| Q20 | RÉSOLUE : chevauchement OUI, max 2 vagues, jamais 2× échelon ≥3, chrono de validation comme juge. |
| Q21 | RÉSOLUE : 14 j ET ≥20 sorties auditées. |
| Q22 | RÉSOLUE : 76 confirmé. |
| Q23 | RÉSOLUE : validé. |
