...
| Toolbox / Outils transverses (2026-07-10) | Audité (lecture seule, 5 façades + système). Inventaire exhaustif sourcé : 55 outils — 25 F1 (5 codés, 15 spécifiés, 4 manquants, 1 dupliqué franc : l'estimation de trajet, spécifiée 2× Daily §5 vs Vector §3.5), 17 F2 (8 codées), 13 F3 (1 seul servi), + 10 dormants. Trou confirmé : notifications (stub vide, 4 specs + Path + Vault en dépendent) ; autres trous : runner (successeur n8n), serving embeddings, privacy gate, geo/H3. Angles morts couverts : Path = consommateur probable de travel/geo (mosquées §7-bis, ghusl) + toolbox.prayer hors des 4 passes ; Vault = moteur de pression (doc 11) + profit hebdo (table inexistante dont dépend la sadaqa). 13 QUESTIONS UTILISATEUR. Ordre confirmé : Toolbox → Pulse → WR → Daily → Vector (+ mini-passe Vault déterministe recommandée). | Voir catalogue | Doublons/trous/couplages/divergences + patches d'étape 0 des 4 specs + liste de migration n8n | Rapports créés: `toolbox/TOOLBOX_CATALOG_DRAFT.md`, `toolbox/TOOLBOX_FINDINGS.md`, `toolbox/EXECUTION_ORDER_PROPOSAL.md`, `toolbox/N8N_INVENTORY.md`, `toolbox/patches/PATCH_{PULSE,WR,DAILY,VECTOR}.md` |
...
- 2026-07-11 : ARCHITECTURE DIGEST livré (`audits/2026-07-11_ARCHITECTURE_DIGEST.md`) — compression de l'écosystème à l'état PRÉ-passes (aucune des 6 passes lancée), entrée unique de la session d'arbitrage d'architecture. AD-1…AD-10, Q14-Q16.
