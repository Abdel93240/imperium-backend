# Doc 05 — Notes historiques à trier (sorties du dictionnaire central)

> Bloc extrait du doc 05 lors de la réécriture du socle (D1, 2026-06-30).
> Fragments de contrats d'API frontend, marqueurs de patches, et bribes de règles
> métier (Vault, Path, Pulse, imperium_events...) qui n'étaient PAS des définitions
> de tables. Conservés ici comme "boîte à pioche" : on y récupérera les bribes UTILES
> au moment de traiter chaque domaine (finance, worship, health...). À supprimer une
> fois tout réintégré.
> NOTE : contient la définition de imperium_events (Event Foundation 23A) = le journal
> DÉPRÉCIÉ (D2). À supprimer, pas à réintégrer.
